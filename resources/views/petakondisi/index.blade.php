...
<body>
    ...
    {{-- DEBUG: tampilkan isi $data yang dikirim ke view --}}
    ...
    <script>
        ...
        data.forEach(item => {
            if (!item.lokasi) return; // skip jika lokasi kosong/null
            let [lat, lng] = item.lokasi.split(',').map(Number);
            if (isNaN(lat) || isNaN(lng)) return; // skip jika bukan koordinat valid
